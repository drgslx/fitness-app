...
def test_reports_only_include_authenticated_users_completed_snapshots():
    ...
    with Session(engine) as db:
        for ident, uid, weight in [(1, 'owner', 62), (2, 'stranger', 999)]:
            db.add(SportType(user_id=uid, name='Sala', is_active=True))
            db.add(Workout(id=ident, user_id=uid, day=date(2026, 9, 25),
                           sport='Sala', title=uid, notes='', exercises=[{'exercise_id': 17,
                           'name': 'Ramat', 'sets': 3, 'reps': 8, 'weight_kg': weight}]))
