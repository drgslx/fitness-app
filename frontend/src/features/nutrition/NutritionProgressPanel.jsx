...
function NutritionResults({ period, anchor }) {
  ...
  if (error) return <p role="alert" className="rounded-lg border border-red-400/30 bg-[#321a18] px-3 py-2 text-[#ffaaaa]">{error}</p>;
  ...
  return <>
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 [&_div]:rounded-lg [&_div]:border [&_div]:border-[#294238] [&_div]:p-3 [&_strong]:block [&_strong]:text-xl [&_strong]:text-accent">
...
export default function NutritionProgressPanel() {
  ...
  return <section className="my-4 grid min-w-0 gap-3 rounded-2xl border border-white/10 bg-surface/90 p-4 shadow-xl">
