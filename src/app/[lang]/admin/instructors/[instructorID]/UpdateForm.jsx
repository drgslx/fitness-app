...
const UpdateForm = ({ instructor }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      categories: instructor?.categories.map((cat) => cat.id) || [],
      description: instructor?.description || "",
      videoImage: instructor?.videoImage || "",
  ...
  useEffect(() => {
    if (instructor) {
      ...
      setValue(
        "categories",
        instructor.categories.map((cat) => cat.id)
      );
  ...
  const onSubmit = async (data) => {
    ...
    try {
        const response = await axios.put(`/api/admin/instructors/${instructor.id}`, {
            name: data.name,
            image: data.image,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-[25px]">
      ...
      <div>
        ...
        <CategorySelect
          value={categories} // Pass the selected category IDs
          onChange={(value) => setValue("categories", value)} // Update the form when categories change
        />
      </div>
      <div>
        <label className="text-[#000000] text-[16px] md:text-[18px] font-semibold block mb-[10px]">
